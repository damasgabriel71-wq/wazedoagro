...
<head>
    <meta charset="UTF-8">
    <title>Categorias — Admin Agro Power</title>
...
<main style="max-width: 1000px; margin: 0 auto; padding: 40px 20px;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px;">
        <div>
            <h1 style="font-size: 1.6rem; margin-bottom: 4px;">🏷️ Categorias</h1>
            <p style="color: #9ca3af;">Gerencie as categorias de produtos.</p>
        </div>
        <a href="/peres/wazedoagro/public/admin/categorias/novo"
    ...
    <div style="background: rgba(0,255,170,0.1); border: 1px solid #00ffaa; color: #00ffaa; padding: 12px; border-radius: 8px; margin-bottom: 20px;">✅ Categoria criada!</div>
    <?php endif; ?>
    <?php if (isset($_GET['atualizado'])): ?>
    <div style="background: rgba(0,255,170,0.1); border: 1px solid #00ffaa; color: #00ffaa; padding: 12px; border-radius: 8px; margin-bottom: 20px;">✅ Categoria atualizada!</div>
    <?php endif; ?>
    <?php if (isset($_GET['deletado'])): ?>
    <div style="background: rgba(239,68,68,0.1); border: 1px solid #ef4444; color: #f87171; padding: 12px; border-radius: 8px; margin-bottom: 20px;">🗑️ Categoria removida!</div>
    <?php endif; ?>
        ...
        <table style="width: 100%; border-collapse: collapse;">
            <thead>
                <tr style="background: #1f2937;">
                    ...
                    <th style="padding: 12px 16px; text-align: left; color: #9ca3af; font-size: 0.8rem; text-transform: uppercase;">Descrição</th>
                    <th style="padding: 12px 16px; text-align: left; color: #9ca3af; font-size: 0.8rem; text-transform: uppercase;">Criado em</th>
            ...
            <tbody>
                ...
                <tr style="border-top: 1px solid rgba(255,255,255,0.05);"
                    ...
                    <td style="padding: 14px 16px; color: #fff; font-weight: 600;"><?= htmlspecialchars($c['nome']) ?></td>
                    <td style="padding: 14px 16px; color: #9ca3af; font-size: 0.85rem;"><?= htmlspecialchars($c['descricao'] ?? '—') ?></td>
                    <td style="padding: 14px 16px; color: #9ca3af; font-size: 0.85rem;"><?= $c['criado_em'] ?></td>
                    <td style="padding: 14px 16px;">
                        <div style="display: flex; gap: 8px;">
                            <a href="/peres/wazedoagro/public/admin/categorias/editar?id=<?= $c['id'] ?>"
                            ...
                            <a href="/peres/wazedoagro/public/admin/categorias/deletar?id=<?= $c['id'] ?>"
                               onclick="return confirm('Remover esta categoria?')"
