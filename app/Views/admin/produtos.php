...
<head>
    <meta charset="UTF-8">
    <title>Produtos — Admin Agro Power</title>
...
<main style="max-width: 1200px; margin: 0 auto; padding: 40px 20px;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px;">
        <div>
            <h1 style="font-size: 1.6rem; margin-bottom: 4px;">📦 Produtos e Estoque</h1>
            <p style="color: #9ca3af;">Gerencie produtos e quantidades em estoque.</p>
        </div>
        <a href="/peres/wazedoagro/public/admin/produtos/novo"
    ...
    <div style="background: rgba(0,255,170,0.1); border: 1px solid #00ffaa; color: #00ffaa; padding: 12px; border-radius: 8px; margin-bottom: 20px;">✅ Produto criado!</div>
    <?php endif; ?>
    <?php if (isset($_GET['atualizado'])): ?>
    <div style="background: rgba(0,255,170,0.1); border: 1px solid #00ffaa; color: #00ffaa; padding: 12px; border-radius: 8px; margin-bottom: 20px;">✅ Produto atualizado!</div>
    <?php endif; ?>
    <?php if (isset($_GET['deletado'])): ?>
    <div style="background: rgba(239,68,68,0.1); border: 1px solid #ef4444; color: #f87171; padding: 12px; border-radius: 8px; margin-bottom: 20px;">🗑️ Produto removido!</div>
    <?php endif; ?>
            ...
            <table style="width: 100%; border-collapse: collapse;">
                <thead>
                    <tr style="background: #1f2937;">
                        ...
                        <th style="padding: 12px 16px; text-align: left; color: #9ca3af; font-size: 0.8rem; text-transform: uppercase;">Categoria</th>
                        <th style="padding: 12px 16px; text-align: left; color: #9ca3af; font-size: 0.8rem; text-transform: uppercase;">Preço</th>
                        <th style="padding: 12px 16px; text-align: left; color: #9ca3af; font-size: 0.8rem; text-transform: uppercase;">Estoque</th>
                ...
                <tbody>
                    ...
                    <tr style="border-top: 1px solid rgba(255,255,255,0.05);"
                        ...
                        <td style="padding: 14px 16px; color: #ffffff; font-size: 0.9rem;"><?= htmlspecialchars($p['nome']) ?></td>
                        <td style="padding: 14px 16px; color: #9ca3af; font-size: 0.85rem;">
                            <?php $cats = ['sal_gado' => '🐄 Sal Gado', 'equinos' => '🐴 Equinos', 'capim' => '🌿 Capim']; ?>
                            <?= $cats[$p['categoria']] ?? $p['categoria'] ?>
                        </td>
                        <td style="padding: 14px 16px; color: #00ffaa; font-size: 0.9rem; font-weight: 600;">R$ <?= number_format($p['preco'], 2, ',', '.') ?></td>
                        <td style="padding: 14px 16px;">
                            <span style="color: <?= $p['estoque'] <= 10 ? '#f87171' : ($p['estoque'] <= 30 ? '#fbbf24' : '#00ffaa') ?>; font-weight: 600;">
                                <?= $p['estoque'] ?> un.
                            </span>
                        </td>
                        <td style="padding: 14px 16px;">
                            <span style="padding: 3px 10px; border-radius: 20px; font-size: 0.75rem; font-weight: 600;
                                background: <?= $p['ativo'] ? 'rgba(0,255,170,0.1)' : 'rgba(239,68,68,0.1)' ?>;
                                color: <?= $p['ativo'] ? '#00ffaa' : '#f87171' ?>;">
                                <?= $p['ativo'] ? 'Ativo' : 'Inativo' ?>
                            </span>
                        </td>
                        <td style="padding: 14px 16px;">
                            <div style="display: flex; gap: 8px;">
                                <a href="/peres/wazedoagro/public/admin/produtos/editar?id=<?= $p['id'] ?>"
                                   style="padding: 6px 12px; background: rgba(0,255,170,0.1); border: 1px solid rgba(0,255,170,0.2); border-radius: 6px; color: #00ffaa; text-decoration: none; font-size: 0.8rem;">
                                    ✏️ Editar
                                </a>
                                <a href="/peres/wazedoagro/public/admin/produtos/deletar?id=<?= $p['id'] ?>"
